...
public class Arrow extends Item {
	...
	private static Vector2 size = new Vector2(0.3f, 0.02f);
	...
	private void stick(PhysicalEntity target, WorldManifold worldManifold) {
		...
		Pose pose = new Pose(contactPoint.cpy().sub(getHeading().rotate90(1).cpy().setLength(size.y / 4f)),
				getHeading().angleRad());
		...
		weld.frequencyHz = 0;
		weld.dampingRatio = 1;
		GameContext.inst().getWorld().createJoint(weld);
	}
	...
	class ArrowStateMachine<T extends Arrow> extends ItemStateMachine<T> {
		...
		ArrowStateMachine(T subject) {
			...
			transitionz.add(new Transition(ProjectileState.FIRED, ProjectileState.FREE,
					() -> !getBody().isAwake() && stickRunnable == null, args -> getSubject().stopShot()));
