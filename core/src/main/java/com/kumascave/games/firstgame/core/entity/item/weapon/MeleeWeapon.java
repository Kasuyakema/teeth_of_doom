...
public abstract class MeleeWeapon extends Handheld {
	...
	class MeleeWeaponStateMachine<T extends MeleeWeapon> extends HandheldStateMachine<T> {
		...
		MeleeWeaponStateMachine(T subject) {
			...
			transitionz
					.add(new Transition(HandheldState.FIXED, HandheldState.SWING, args -> getSubject().startSwing()));
			transitionz.add(new Transition(HandheldState.SWING, HandheldState.FIXED, args -> getSubject().stopSwing()));
